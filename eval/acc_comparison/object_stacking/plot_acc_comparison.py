...
TICK_FONTSIZE = 14
LAYOUT_SPECS = {
    "2x5": {"nrows": 2, "ncols": 5, "figsize": (18.0, 6.6), "wspace": 0.35, "hspace": 0.18},
    "5x2": {"nrows": 5, "ncols": 2, "figsize": (6.0, 8.8), "wspace": 0.19, "hspace": 0.0},
    "1x10": {"nrows": 1, "ncols": 10, "figsize": (23.0, 2.8), "wspace": 0.16, "hspace": 0.0},
}
METHOD_ORDER = [
    "Ours",
    "MAPPO",
...
def main():
    ...
    for idx, ax in enumerate(axes):
        ...
        ax.set_title(f"Environment {idx + 1}", fontsize=TITLE_FONTSIZE)
        ...
        ax.tick_params(axis="x", labelsize=TICK_FONTSIZE, length=0, pad=8 if args.layout == "5x2" else 6)
        ax.tick_params(axis="y", labelsize=TICK_FONTSIZE, length=0, pad=5 if args.layout == "5x2" else 4)
        for spine in ("top", "right", "bottom", "left"):
            ax.spines[spine].set_visible(True)
            ax.spines[spine].set_linewidth(0.9)
        ax.set_box_aspect(1.25 if args.layout == "1x10" else (0.46 if args.layout == "5x2" else 0.78))
        if idx // ncols == nrows - 1:
            ax.set_xlabel("Time (minutes)", fontsize=LABEL_FONTSIZE, labelpad=8 if args.layout == "5x2" else 6)
        if idx % ncols == 0:
            ax.set_ylabel("Accuracy", fontsize=LABEL_FONTSIZE, labelpad=8 if args.layout == "5x2" else 6)

    bottom = 0.03 if args.layout == "5x2" else (0.16 if args.layout == "1x10" else 0.08)
    top = 0.99 if args.layout == "5x2" else (0.88 if args.layout == "1x10" else 0.94)
    fig.subplots_adjust(
