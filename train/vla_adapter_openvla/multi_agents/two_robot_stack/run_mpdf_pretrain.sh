...
MPDF_EPS="${MPDF_EPS:-1e-5}"
MPDF_GROUP_SIZE="${MPDF_GROUP_SIZE:-0}"
MPDF_RANK_TEMPERATURE="${MPDF_RANK_TEMPERATURE:-1.0}"
MPDF_KL_COEF="${MPDF_KL_COEF:-0.0}"
...
if [[ "${MODEL_BACKBONE}" != "smolvla" && "${MODEL_BACKBONE}" != "mixed_tiny_vla_openvla" && "${MODEL_BACKBONE}" != "mixed_tiny_vla_smolvla" ]]; then
  echo "MODEL_BACKBONE must be smolvla, mixed_tiny_vla_openvla, or legacy mixed_tiny_vla_smolvla for planner pretraining." >&2
  exit 1
fi

CMD=(
  "${PYTHON_BIN}" ${PROJECT_ROOT}/train/vla_adapter_openvla/multi_agents/two_robot_stack/mpdf_pretrain.py
  ...
  --target-kl "${TARGET_KL}"
  --mpdf-eps "${MPDF_EPS}"
  --mpdf-group-size "${MPDF_GROUP_SIZE}"
  --mpdf-rank-temperature "${MPDF_RANK_TEMPERATURE}"
  --mpdf-kl-coef "${MPDF_KL_COEF}"
  --mpdf-ent-coef "${MPDF_ENT_COEF}"
