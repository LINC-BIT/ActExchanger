...
from train.vla_adapter_openvla.multi_agents.two_robot_stack.model import MultiAgentVLAAdapterMAPPOAgent
...
class FrozenBackboneAdapterAgent(MultiAgentVLAAdapterMAPPOAgent):
    def __init__(self, *args, **kwargs):
        ...
        if model_backbone == CANONICAL_MIXED_MODEL_BACKBONE:
            raise ValueError(
                "FrozenBackboneAdapterAgent currently supports only model_backbone=tiny or openvla. "
                f"{CANONICAL_MIXED_MODEL_BACKBONE} needs a dedicated adapter path."
            )
    ...
    def load_checkpoint_state_dict(self, state_dict: Dict[str, torch.Tensor]) -> None:
        ...
        if has_mixed_actor_prefix and not has_single_actor_prefix:
            raise RuntimeError(
                ...
                "Use a matching tiny/openvla checkpoint under vla_adapter_openvla/, or add a dedicated mixed-backbone adapter wrapper."
            )
...
class FrozenMixedBackboneAdapterAgent(MixedTinyVLAAdapterOpenVLASFTAgent):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
