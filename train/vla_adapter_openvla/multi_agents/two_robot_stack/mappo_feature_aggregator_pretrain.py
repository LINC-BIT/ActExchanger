...
from train.vla_adapter_openvla.multi_agents.two_robot_stack.mixed_sft_agent import (
    ...
    LEGACY_MIXED_MODEL_BACKBONE,
    MixedTinyVLAAdapterOpenVLASFTAgent,
    is_mixed_model_backbone,
)
from train.vla_adapter_openvla.multi_agents.two_robot_stack.model import (
...
MODEL_NAME = "vla_adapter_openvla"
ENV_KWARGS_LIST = [
    {
...
def get_model_name(args) -> str:
    if is_mixed_model_backbone(args.model_backbone):
        return "vla_adapter_openvla_mappo"
    return MODEL_NAME
...
def _slice_tree(x, start: int, end: int):
    if torch.is_tensor(x) or isinstance(x, np.ndarray):
        return x[start:end]
    if isinstance(x, dict):
        return {k: _slice_tree(v, start, end) for k, v in x.items()}
    if isinstance(x, list):
        return [_slice_tree(v, start, end) for v in x]
    if isinstance(x, tuple):
        return tuple(_slice_tree(v, start, end) for v in x)
    return x


def _concat_trees(items):
    if len(items) == 0:
        return None
    first = items[0]
    if first is None:
    ...
    if isinstance(first, dict):
        return {k: _concat_trees([item[k] for item in items]) for k in first.keys()}
    if isinstance(first, list):
        return [_concat_trees([item[idx] for item in items]) for idx in range(len(first))]
    if isinstance(first, tuple):
        return tuple(_concat_trees([item[idx] for item in items]) for idx in range(len(first)))
    return first


def _zeros_like_tree(template, batch_size: int):
    if template is None:
        return None
    if torch.is_tensor(template):
        shape = (batch_size,) + tuple(template.shape[1:]) if template.ndim > 0 else (batch_size,)
        return torch.zeros(shape, dtype=template.dtype, device=template.device)
    if isinstance(template, np.ndarray):
        shape = (batch_size,) + template.shape[1:] if template.ndim > 0 else (batch_size,)
        return np.zeros(shape, dtype=template.dtype)
    if isinstance(template, dict):
        return {k: _zeros_like_tree(v, batch_size) for k, v in template.items()}
    if isinstance(template, list):
        return [_zeros_like_tree(v, batch_size) for v in template]
    if isinstance(template, tuple):
        return tuple(_zeros_like_tree(v, batch_size) for v in template)
    return template


def _concat_info_trees(items, batch_sizes):
    valid_items = [item for item in items if item is not None]
    if len(valid_items) == 0:
        return None

    template = valid_items[0]
    if torch.is_tensor(template) or isinstance(template, np.ndarray):
        filled_items = [
            item if item is not None else _zeros_like_tree(template, batch_size)
            for item, batch_size in zip(items, batch_sizes)
        ]
        return _concat_trees(filled_items)
    ...
    if isinstance(template, dict):
        ...
        for key in keys:
            ...
            child_template = None
            for item in items:
                child = None if item is None else item.get(key)
                child_items.append(child)
                if child is not None and child_template is None:
                    child_template = child
            filled_child_items = [
                child if child is not None else _zeros_like_tree(child_template, batch_size)
                for child, batch_size in zip(child_items, batch_sizes)
            ]
            merged[key] = _concat_info_trees(filled_child_items, batch_sizes)
        return merged

    if isinstance(template, list):
        return [
            _concat_info_trees(
                [None if item is None else item[idx] for item in items],
                batch_sizes,
            )
...
class ConcatManiSkillVectorEnv:
    def __init__(self, envs, profile_specs):
        if len(envs) == 0:
            raise ValueError("ConcatManiSkillVectorEnv requires at least one sub-env")
        self.envs = envs
        self.profile_specs = profile_specs
        self.env_counts = [env.num_envs for env in envs]
        self.num_envs = int(sum(self.env_counts))
        self.single_observation_space = envs[0].single_observation_space
        self.single_action_space = envs[0].single_action_space
        self.observation_space = getattr(envs[0], "observation_space", None)
        self.action_space = getattr(envs[0], "action_space", None)
        self.device = getattr(envs[0], "device", None)

    def reset(self, seed=None, options=None):
        obs_list = []
        info_list = []
        for idx, env in enumerate(self.envs):
            env_seed = None if seed is None else int(seed) + idx * 100003
            obs, infos = env.reset(seed=env_seed, options=options)
            obs_list.append(obs)
            info_list.append(infos)
        return _concat_trees(obs_list), _concat_info_trees(info_list, self.env_counts)
    ...
    def step(self, actions):
        ...
        info_list = []
        start = 0
        for env, env_count in zip(self.envs, self.env_counts):
            end = start + env_count
            env_actions = _slice_tree(actions, start, end)
            obs, rewards, terminations, truncations, infos = env.step(env_actions)
            obs_list.append(obs)
            reward_list.append(rewards)
            termination_list.append(terminations)
            truncation_list.append(truncations)
            info_list.append(infos)
            start = end
        return (
            _concat_trees(obs_list),
            _concat_trees(reward_list),
            _concat_trees(termination_list),
            _concat_trees(truncation_list),
            _concat_info_trees(info_list, self.env_counts),
        )
...
def build_profile_env_kwargs(base_env_kwargs, env_profile):
    env_kwargs = dict(base_env_kwargs)
    env_kwargs.update({k: v for k, v in env_profile.items() if k != "name"})
    return env_kwargs


def make_train_env_for_profile(args, env_profile, base_env_kwargs):
    train_env_kwargs = build_profile_env_kwargs(base_env_kwargs, env_profile)
    train_env_kwargs["render_mode"] = "none"
    train_env_kwargs["sim_backend"] = "physx_cuda"
    envs = gym.make(args.task_name, num_envs=args.num_envs, **train_env_kwargs)
    ...
    return envs, env_profile["name"]


def make_eval_envs_for_profile(args, ckpt, env_profile, base_env_kwargs, eval_agent_dir=None):
    eval_env_kwargs = build_profile_env_kwargs(base_env_kwargs, env_profile)
    eval_env_kwargs["render_mode"] = "rgb_array"

    video_root = (
        os.path.join(eval_agent_dir, "videos_eval")
        if eval_agent_dir is not None
        else f"{ckpt['video_dir']}_train"
    )
    env_name = env_profile["name"]
    eval_envs = make_eval_envs(
        env_id=args.task_name,
        num_envs=args.num_eval_envs,
        sim_backend="gpu",
        env_kwargs=eval_env_kwargs,
        video_dir=os.path.join(video_root, env_name),
    )
    eval_envs.reset(seed=args.seed)
    return eval_envs, env_name
...
def _split_count_evenly(total_count: int, num_parts: int):
    if num_parts <= 0:
        raise ValueError(f"num_parts must be positive, got {num_parts}")
    base = total_count // num_parts
    remainder = total_count % num_parts
    return [base + (1 if idx < remainder else 0) for idx in range(num_parts)]


def _split_env_count_into_shards(env_count: int, shard_size: int):
    if env_count < 0:
        raise ValueError(f"env_count must be non-negative, got {env_count}")
    if shard_size <= 0:
        raise ValueError(f"shard_size must be positive, got {shard_size}")
    shards = []
    remaining = int(env_count)
    while remaining > 0:
        current_shard = min(remaining, shard_size)
        shards.append(current_shard)
        remaining -= current_shard
    return shards


def get_mixed_env_groups(args):
    return [
        ("clean", args.mixed_env_clean_ratio, [ENV_KWARGS_LIST[0], ENV_KWARGS_LIST[4]]),
        ("mild", args.mixed_env_mild_ratio, [ENV_KWARGS_LIST[1], ENV_KWARGS_LIST[2]]),
        ("hard", args.mixed_env_hard_ratio, [ENV_KWARGS_LIST[3]]),
    ]


def build_mixed_train_envs(args, base_env_kwargs):
    group_specs = get_mixed_env_groups(args)
    group_counts = _allocate_env_counts(args.num_envs, [ratio for _, ratio, _ in group_specs])
    shard_size = int(args.mixed_env_shard_size)

    built_envs = []
    built_specs = []
    for (group_name, _, env_profiles), group_count in zip(group_specs, group_counts):
        if group_count <= 0:
            continue
        per_profile_counts = _split_count_evenly(group_count, len(env_profiles))
        for env_profile, profile_count in zip(env_profiles, per_profile_counts):
            if profile_count <= 0:
                continue
            profile_env_kwargs = build_profile_env_kwargs(base_env_kwargs, env_profile)
            profile_env_kwargs["render_mode"] = "none"
            profile_env_kwargs["sim_backend"] = "physx_cuda"
            shard_counts = _split_env_count_into_shards(profile_count, shard_size)
            for shard_idx, shard_env_count in enumerate(shard_counts):
                env = gym.make(args.task_name, num_envs=shard_env_count, **profile_env_kwargs)
                ...
                built_specs.append((group_name, env_profile["name"], shard_idx, shard_env_count))

    if len(built_envs) == 0:
        raise ValueError("Failed to build mixed train envs: no sub-envs were created")

    profile_summary = ", ".join(
        f"{group_name}/{profile_name}[{shard_idx}]={env_count}"
        for group_name, profile_name, shard_idx, env_count in built_specs
    )
    print(f"[MixedEnv] Build-time train env split (shard_size={shard_size}): {profile_summary}")
    return ConcatManiSkillVectorEnv(built_envs, built_specs), "mixed_train_envs"
...
def build_clients(args, agent, infos, agent_names, device):
    if is_mixed_model_backbone(args.model_backbone):
        vla_action_dim = int(agent.action_dims[agent.vla_agent_name])
        smolvla_action_dim = int(agent.action_dims[agent.smolvla_agent_name])
        return {
            agent.vla_agent_name: VLAClientForMultiAgent(
                ...
                num_action_positions=vla_action_dim,
                device=device,
                local_action_dim=vla_action_dim,
                max_episode_steps=args.max_episode_steps,
            ...
            agent.smolvla_agent_name: VLAClientForMultiAgent(
                ...
                num_action_positions=smolvla_action_dim,
                device=device,
                local_action_dim=smolvla_action_dim,
                max_episode_steps=args.max_episode_steps,
                feature_aggregator_attention_num_heads=args.feature_aggregator_attention_num_heads,
                feature_aggregator_gate_type=args.feature_aggregator_gate_type,
                feature_aggregator_gate_activation=args.feature_aggregator_gate_activation,
...
def main(args):
    ...
    collate_fn = make_collate_fn(agent_names)
    if len(set(infos["action_dims"].values())) != 1 and not is_mixed_model_backbone(args.model_backbone):
        raise ValueError(
            f"Heterogeneous action dimensions require a mixed backbone for {args.task_name}. "
            f"Got action_dims={infos['action_dims']} with model_backbone={args.model_backbone}."
        )
    ...
    if is_mixed_model_backbone(args.model_backbone):
        ...
        agent = MixedTinyVLAAdapterOpenVLASFTAgent(
            **infos,
        ...
        if getattr(args, "full_kl_coef", 0.0) > 0 or getattr(args, "log_full_kl", False):
            if accelerator.is_main_process:
                print(
                    f"[Config] {CANONICAL_MIXED_MODEL_BACKBONE} disables full token-distribution KL; "
    ...
    if args.enable_mixed_train_envs:
        print(
            "[MixedEnv] Using build-time mixed train env split: "
    ...
    if args.evaluate_mode:
        eval_envs, current_env_name = make_eval_envs_for_profile(
            args,
            ckpt,
            env_profile=ENV_KWARGS_LIST[0],
            base_env_kwargs=base_env_kwargs,
            eval_agent_dir=args.eval_agent_dir,
        )
    ...
    if args.enable_mixed_train_envs:
        envs, train_env_name = build_mixed_train_envs(args, base_env_kwargs)
    else:
        envs, train_env_name = make_train_env_for_profile(args, ENV_KWARGS_LIST[0], base_env_kwargs)
    eval_envs, eval_env_name = make_eval_envs_for_profile(
        args,
        ckpt,
        env_profile=ENV_KWARGS_LIST[0],
        base_env_kwargs=base_env_kwargs,
    )
    print(f"[AGPretrain] train_env={train_env_name}, eval_env={eval_env_name}")
...
def parse_args():
    ...
    parser.add_argument("--ckpt-task-name", type=str, default="TwoRobotStackCubeUR10e-v1_ag")
    ...
    parser.add_argument(
        "--model-backbone",
        type=str,
        default=CANONICAL_MIXED_MODEL_BACKBONE,
        choices=["openvla", "tiny", CANONICAL_MIXED_MODEL_BACKBONE, LEGACY_MIXED_MODEL_BACKBONE],
        help="Choose the original OpenVLA adapter backbone, the local tiny autoregressive VLA backbone, or the mixed OpenVLA adapter backbone. The legacy alias mixed_tiny_vla_smolvla remains supported.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="maximum number of sub-envs packed into one mixed-profile shard",
    )
    parser.add_argument(
        "--log-timing",
        action="store_true",
