...
sys.path.append(str(Path(__file__).resolve().parents[4] / "train/toy_cnn/multi_agents/two_robot_pick"))
...
from train.toy_cnn.multi_agents.two_robot_stack.mappo_pretrain import HeteroMAPPOAgent
...
MODEL_NAME = "vla_adapter_openvla_sft"
DEFAULT_EXPERT_AGENT_DIR = "ckpt/TwoRobotStackCubeUR10e-v1/ppo/panda_ur10e_panda_gripper/toy_cnn/20260724-130231"


def get_model_name(args) -> str:
    if is_mixed_model_backbone(args.model_backbone):
        return "vla_adapter_openvla_sft"
    return MODEL_NAME
...
def make_sample_fn(agent_names: Sequence[str], agent: MultiAgentVLAAdapterMAPPOAgent, deterministic=True):
    def sample_fn(obs):
...
def build_expert_batch_from_raw_obs(obs: Dict[str, Any], device: torch.device) -> Dict[str, torch.Tensor]:
    ...
    for agent_name in obs["agent"].keys():
        ...
        tcp_to_cube_key = "left_arm_tcp_to_cubeA_pos" if is_left else "right_arm_tcp_to_cubeB_pos"
        flat_state = common.flatten_state_dict(
            {
                "cubeA_pose": obs["extra"]["cubeA_pose"],
                "cubeB_pose": obs["extra"]["cubeB_pose"],
...
def get_vla_agent_info(args) -> Dict[str, Any]:
    ...
    state_dims = {name: int(batch[f"agent_states_{name}"].shape[-1]) for name in agent_names}
    unique_state_dims = sorted(set(state_dims.values()))
    if len(unique_state_dims) != 1:
        raise ValueError(f"VLA agent expects uniform state dims, got {state_dims}")
    action_dims = {
        agent_name: int(env.single_action_space[agent_name].shape[0])
        for agent_name in agent_names
    }
    info = {
        "agent_names": agent_names,
        "state_dim": unique_state_dims[0],
...
def get_expert_agent_info(args, device: torch.device) -> Dict[str, Any]:
    ...
    info = {
        "agent_names": agent_names,
        "state_dims": {
            agent_name: int(batch[f"agent_states_{agent_name}"].shape[-1])
            for agent_name in agent_names
        },
        "global_state_dim": batch["global_state"].shape[-1],
        "action_dims": {
...
def resolve_expert_checkpoint_path(expert_agent_dir: str) -> str:
    candidate = Path(expert_agent_dir)
    if candidate.is_file():
        return str(candidate)
    for filename in ("best_agent.pt", "latest_agent.pt"):
        ckpt_path = candidate / filename
        if ckpt_path.exists():
            return str(ckpt_path)
    raise FileNotFoundError(
        f"Expert checkpoint not found under {expert_agent_dir}. Expected a checkpoint file or a directory "
        "containing best_agent.pt or latest_agent.pt."
    )


def load_expert_agent(args, device: torch.device) -> HeteroMAPPOAgent:
    ...
    expert_ckpt_path = resolve_expert_checkpoint_path(args.expert_agent_dir)
    print(f"[Expert] loading checkpoint from {expert_ckpt_path}")
    agent.load_state_dict(torch.load(expert_ckpt_path, map_location="cpu"))
    set_sparsity(agent, float(args.expert_max_sparsity))
...
def collect_expert_trajectories(args, ckpt: Dict[str, str], expert_agent: torch.nn.Module) -> str:
...
def load_sft_dataset_from_trajectories(
    ...
    if os.path.exists(cache_path) and not args.force_rebuild_dataset_cache:
        ...
        cache_valid = True
        for name in agent_names:
            expected_dim = int(action_dims[name])
            key = f"actions_{name}"
            if key not in dataset or int(dataset[key].shape[-1]) != expected_dim:
                cache_valid = False
                print(
                    f"[Dataset] cache action dim mismatch for {name}: "
                    f"got {None if key not in dataset else int(dataset[key].shape[-1])}, expected {expected_dim}; rebuilding cache."
                )
                break
        if cache_valid:
            return dataset
    ...
    with h5py.File(trajectory_h5_path, "r") as h5_file:
        for episode in tqdm(success_episodes, ascii=True, desc="Build SFT Dataset"):
            ...
            if isinstance(action_node, h5py.Dataset):
                ...
                start = 0
                for name in agent_names:
                    action_dim = int(action_dims[name])
                    end = start + action_dim
                    actions[name] = flat_actions[:, start:end]
                    start = end
                if start != flat_actions.shape[-1]:
                    raise ValueError(
                        f"Flattened action dim mismatch: consumed {start}, "
                        f"but trajectory stores {flat_actions.shape[-1]}"
                    )
            else:
...
def build_sft_optimizer(args, agent: MultiAgentVLAAdapterMAPPOAgent) -> torch.optim.Optimizer:
    if isinstance(agent, MixedTinyVLAAdapterOpenVLASFTAgent):
        param_groups = [
            {
...
def add_action_bins_to_dataset(dataset: Dict[str, Any], agent: MultiAgentVLAAdapterMAPPOAgent, chunk_size: int) -> None:
...
def train_sft(args):
    ...
    while global_updates < total_iters:
        ...
        with torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=autocast_enabled):
            ...
            nll_loss = (-mean_log_prob) / agent.action_dim
            entropy_bonus = args.entropy_coef * mean_entropy
            bc_loss = nll_loss - entropy_bonus
        ...
        writer.add_scalar("train/nll_loss_step", float(nll_loss.detach().item()), global_updates)
        writer.add_scalar("train/entropy_bonus_step", float(entropy_bonus.detach().item()), global_updates)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Choose the original OpenVLA adapter backbone, the local tiny autoregressive VLA backbone, or the mixed OpenVLA adapter backbone. model_dir is required only for openvla. The legacy alias mixed_tiny_vla_smolvla remains supported.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Entropy regularization coefficient for SFT to avoid premature policy collapse before PPO.",
    )
