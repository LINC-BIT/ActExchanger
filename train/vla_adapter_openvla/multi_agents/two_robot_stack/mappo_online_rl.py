...
MODEL_NAME = "vla_adapter_openvla_mappo"
ALGO_NAME = "ppo"
AGENT_CLS = MixedTinyVLAAdapterOpenVLASFTAgent
...
def main(args):
    ...
    if os.path.exists(ckpt["latest_opt"]):
        ...
        if resume_info["resumed"] and resume_info["used_best_agent"]:
            ...
            _, best_elapsed_minutes = infer_best_step_from_best_meta(ckpt["best_meta"])
            if best_elapsed_minutes is not None:
                elapsed_minutes_offset = float(best_elapsed_minutes)
            else:
                elapsed_minutes_offset = float(latest_opt_payload.get("elapsed_minutes", 0.0))
