...
from train.vla_adapter_openvla.multi_agents.two_robot_stack.tiny_vla import SharedTinyVLA4DActor

from prismatic.vla.action_tokenizer import ActionTokenizer


TASK_PROMPT = (
    "coordinate two robot arms to pick up the red cube, stack it onto the green cube, and release it stably."
)
...
def build_agent_role_prompt(agent_name: str) -> str:
    ...
    return (
        ...
        "stack it onto the green cube, and release it stably."
    )
...
def extract_agent_state_from_obs(obs: Dict[str, Any], agent_name: str) -> torch.Tensor:
    ...
    if qpos.shape[-1] < 9:
        qpos = F.pad(qpos, (0, 9 - qpos.shape[-1]))
    if qvel.shape[-1] < 9:
        qvel = F.pad(qvel, (0, 9 - qvel.shape[-1]))
    ...
    cube_a_pose = _ensure_2d(_as_tensor(obs["extra"]["cubeA_pose"], dtype=torch.float32))
    cube_b_pose = _ensure_2d(_as_tensor(obs["extra"]["cubeB_pose"], dtype=torch.float32))
    stage = _ensure_2d(_as_tensor(obs["extra"]["stage"], dtype=torch.float32), batch_size=batch_size)

    return torch.cat(
        [qpos, qvel, tcp, tcp_to_cube, cube_a_pose, cube_b_pose, stage],
        dim=-1,
    )
...
def extract_global_state_from_obs(obs: Dict[str, Any], agent_names: List[str]) -> torch.Tensor:
    ...
    cube_a_pose = _ensure_2d(_as_tensor(obs["extra"]["cubeA_pose"], dtype=torch.float32))
    cube_b_pose = _ensure_2d(_as_tensor(obs["extra"]["cubeB_pose"], dtype=torch.float32))
    left_tcp_to_cube = _ensure_2d(_as_tensor(obs["extra"]["left_arm_tcp_to_cubeA_pos"], dtype=torch.float32))
    right_tcp_to_cube = _ensure_2d(_as_tensor(obs["extra"]["right_arm_tcp_to_cubeB_pos"], dtype=torch.float32))
    cube_a_to_cube_b = _ensure_2d(_as_tensor(obs["extra"]["cubeA_to_cubeB_pos"], dtype=torch.float32))
    goal_region_pos = _ensure_2d(_as_tensor(obs["extra"]["goal_region_pos"], dtype=torch.float32))
    stage = _ensure_2d(_as_tensor(obs["extra"]["stage"], dtype=torch.float32), batch_size=batch_size)
