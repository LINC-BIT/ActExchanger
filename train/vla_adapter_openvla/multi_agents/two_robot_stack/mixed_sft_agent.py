...
from train.vla_adapter_openvla.multi_agents.two_robot_stack.tiny_vla import (
    MLPProjector,
    SharedTinyVLA4DActor,
    TinyVLABackbone,
)

CANONICAL_MIXED_MODEL_BACKBONE = "mixed_tiny_vla_openvla"
LEGACY_MIXED_MODEL_BACKBONE = "mixed_tiny_vla_smolvla"
MIXED_MODEL_BACKBONES = {
    CANONICAL_MIXED_MODEL_BACKBONE,
    LEGACY_MIXED_MODEL_BACKBONE,
}


def is_mixed_model_backbone(model_backbone: Optional[str]) -> bool:
    return model_backbone in MIXED_MODEL_BACKBONES
...
class MixedTinyVLAAdapterOpenVLASFTAgent(nn.Module):
    def __init__(
        self,
        ...
        self.action_dims = {
            name: int(action_dims[name] if action_dims is not None else action_dim)
            for name in self.agent_names
        }
        self.action_dim = max(self.action_dims.values())
        self.vla_agent_name = self.agent_names[0]
        self.smolvla_agent_name = self.agent_names[1]
        self.vla_action_dim = int(self.action_dims[self.vla_agent_name])
        self.smolvla_action_dim = int(self.action_dims[self.smolvla_agent_name])
        self.smolvla_num_action_bins = max(int(tiny_num_action_bins), 2)

        self.vla_actor = SharedTinyVLA4DActor(
            model_dir=model_dir,
            state_dim=self.state_dim,
            env_action_dim=self.vla_action_dim,
        ...
        self.smolvla_actor = TinySmolVLAContinuousActor(
            state_dim=self.state_dim,
            action_dim=self.smolvla_action_dim,
        ...
        self.vla_actor_action_position_placeholders = nn.ModuleList(
            [nn.Identity() for _ in range(self.vla_action_dim)]
        )
        ...
        self.smolvla_actor_action_position_placeholders = nn.ModuleList(
            [nn.Identity() for _ in range(self.smolvla_action_dim)]
        )
    ...
    def load_checkpoint_state_dict(self, state_dict):
        ...
        for key, value in state_dict.items():
            ...
            matched[key] = value
        if not matched:
            raise RuntimeError("No compatible parameters found for MixedTinyVLAAdapterOpenVLASFTAgent")
        ...
        print(
            "[Checkpoint] MixedTinyVLAAdapterOpenVLASFTAgent "
            f"matched={len(matched)} skipped={len(skipped)} missing={len(missing)}"
        )


def build_mixed_mappo_optimizer(args, agent: MixedTinyVLAAdapterOpenVLASFTAgent) -> torch.optim.Optimizer:
    param_groups = [
        {
...
MixedTinyVLAAdapterSmolVLASFTAgent = MixedTinyVLAAdapterOpenVLASFTAgent
