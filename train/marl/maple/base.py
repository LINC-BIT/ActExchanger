...
from train.marl.mappo.base import get_trainable_optimizer_parameters
...
def _collect_policy_output(agent, batch):
    supports_action_bins = _agent_supports_kwarg(agent, "return_action_bins")
    if supports_action_bins:
        output = agent.get_action_and_value(batch, return_action_bins=True)
    else:
        output = agent.get_action_and_value(batch)
    return _normalize_rollout_output(agent, output)
...
def _maybe_extract_future_state_target(batch_like):
    if not isinstance(batch_like, Mapping):
        return None
    states = batch_like.get("states")
    if states is None:
        return None
    if isinstance(states, np.ndarray):
        return torch.from_numpy(states).to(dtype=torch.float32)
    if isinstance(states, torch.Tensor):
        return states.detach().to(dtype=torch.float32)
...
def collect_rollout(args, agent, collate_fn, envs, next_obs, next_done, accelerator, writer, global_step, clients=None):
    ...
    agent_names = list(getattr(agent, "actor_heads", {}).keys())
    if not agent_names:
        agent_names = ["ego"]
        agent.actor_heads = {agent_names[0]: None}
    ...
    if _maybe_extract_future_state_target(initial_batch) is not None:
        target_shape = tuple(_maybe_extract_future_state_target(initial_batch).shape[1:])
        future_state_targets = torch.zeros((T, N) + target_shape, dtype=torch.float32, device=device)
    ...
    for step in pbar:
        ...
        with torch.no_grad():
            ...
            actions_dict, logp_dict, ent_dict, value, rollout_action_dict, using_action_bins = _collect_policy_output(
                agent,
                batch,
            )
        ...
        for name in actions_dict.keys():
            if using_action_bins:
                ...
                actions[name][step] = np.asarray(rollout_action_dict[name], dtype=np.float32)
            logprobs[name][step] = logp_dict[name].detach().to(device)
            entropies[name][step] = ent_dict[name].detach().to(device)

        if hasattr(envs.single_action_space, "spaces"):
            env_action = actions_dict
        else:
            env_action = actions_dict[agent_names[0]]
        ...
        if future_state_targets is not None:
            ...
            future_state_targets[step] = _to_device_tensor(
                _maybe_extract_future_state_target(next_batch),
                device,
                dtype=torch.float32,
            )
    ...
    meta = {
        ...
        "using_action_bins": using_action_bins,
    }
    return (
        obs,
        rollout_actions,
...
def _future_state_aux_loss(args, aux_outputs: Mapping[str, Any], meta: Mapping[str, Any], mb_inds, device):
    pred = aux_outputs.get("future_state")
    target = _meta_vector(meta, "future_state_target", mb_inds, device, dtype=torch.float32)
    ...
    mask = _meta_vector(meta, "future_state_mask", mb_inds, device, dtype=torch.float32)
    error = pred - target
...
def maple_sft_update(args, agent, optimizer, data, collate_fn, accelerator, stage, writer):
    ...
    B = len(b_obs)
    b_inds = np.arange(B)
    pbar = tqdm(total=args.update_epochs * max(1, B // args.minibatch_size), desc=stage, ascii=True)
    ...
    for _ in range(args.update_epochs):
        ...
        for start in range(0, B, args.minibatch_size):
            ...
            for name in new_logp.keys():
                bc_loss = bc_loss + (-new_logp[name].mean())
                entropy_loss = entropy_loss + entropy[name].mean()

            future_state_loss = _future_state_aux_loss(args, aux_outputs, meta, mb_inds, device)
    ...
    for _ in range(args.update_epochs):
        ...
        for start in range(0, B, args.minibatch_size):
            ...
            for name in new_logp.keys():
                ...
                pg1 = -adv * ratio
                pg2 = -adv * torch.clamp(ratio, 1 - args.clip_eps, 1 + args.clip_eps)
                policy_loss = policy_loss + torch.max(pg1, pg2).mean()
                entropy_loss = entropy_loss + entropy[name].mean()
                bc_loss = bc_loss + (-new_logp[name].mean())
...
def _extract_future_state_target_from_batch(batch_like):
    ...
    if "global_state" in batch_like and batch_like["global_state"] is not None:
        return _to_device_tensor(batch_like["global_state"], torch.device("cpu"), dtype=torch.float32)
    if "states" in batch_like and batch_like["states"] is not None:
        return _to_device_tensor(batch_like["states"], torch.device("cpu"), dtype=torch.float32)
    return None
...
def _build_stepwise_group_meta(args, rewards: torch.Tensor, dones: torch.Tensor, device: torch.device):
    ...
    group_ids = torch.stack(
        [local_group_ids + step * groups_per_step for step in range(T)],
        dim=0,
    )
    ...
    flat_returns = joint_returns.reshape(-1)
...
def _future_state_aux_loss_multi(args, aux_outputs: Mapping[str, Any], meta: Mapping[str, Any], mb_inds, batch_size: int, device):
    pred = aux_outputs.get("future_state")
...
def collect_rollout(args, agent, collate_fn, envs, next_obs, next_done, accelerator, writer, global_step, clients=None):
    ...
    obs = DictArray((T, N), envs.single_observation_space, device=device)
    agent_names = list(getattr(agent, "actor_heads", {}).keys())
    if not agent_names and hasattr(agent, "agent_names"):
        agent_names = list(agent.agent_names)

    actions = {
        name: np.zeros((T, N) + envs.single_action_space[name].shape, dtype=np.float32)
        for name in agent_names
    }
    action_bins = {
...
def maple_sft_update(args, agent, optimizer, data, collate_fn, accelerator, stage, writer):
    ...
    for _ in range(args.update_epochs):
        ...
        for start in range(0, batch_size, args.minibatch_size):
            ...
            future_state_loss = _future_state_aux_loss_multi(
                args,
                aux_outputs,
                meta,
                mb_inds,
...
def maple_update_on_policy(args, agent, optimizer, data, collate_fn, accelerator, stage, writer, rollouts_num_aft_env_change=None):
    ...
    future_state_coef = float(getattr(args, "maple_future_state_coef", 0.0))
    bc_coef = float(getattr(args, "maple_rl_bc_coef", 0.0))
    entropy_coef = float(getattr(args, "maple_ent_coef", getattr(args, "magrpo_ent_coef", getattr(args, "ent_coef", 0.0))))
    ...
    for _ in range(args.update_epochs):
        ...
        for start in range(0, batch_size, args.minibatch_size):
            ...
            loss = policy_loss - entropy_coef * entropy_loss + bc_coef * bc_loss + future_state_coef * future_state_loss
            loss = loss / args.grad_accum_steps
            accelerator.backward(loss)
            accum_count += 1
