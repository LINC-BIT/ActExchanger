...
def parse_args():
    ...
    parser.add_argument("--model-backbone", type=str, default="mixed_tiny_vla_smolvla")
    ...
    for idx, arg in enumerate(argv):
        if skip_next:
            skip_next = False
            continue
        if arg == "--ckpt-task-name" or arg == "--env-change-time-points" or arg == "--max-time" or arg == "--model-backbone":
            skip_next = True
            continue
