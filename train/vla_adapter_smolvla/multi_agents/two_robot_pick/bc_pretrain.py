...
MODEL_NAME = "vla_adapter_smolvla_sft"
...
def get_model_name(args) -> str:
    if args.model_backbone == "mixed_tiny_vla_smolvla":
        return "vla_adapter_smolvla_sft"
    return MODEL_NAME
...
def collect_expert_trajectories(args, ckpt: Dict[str, str], expert_agent: MAPPOAgent) -> str:
...
def build_sft_optimizer(args, agent: MultiAgentVLAAdapterMAPPOAgent) -> torch.optim.Optimizer:
    if isinstance(agent, MixedTinyVLAAdapterSmolVLASFTAgent):
        param_groups = [
            {
