...
from train.vla_adapter_smolvla_efficientvla_tinyvla_flower.multi_agents.pass_object_five_robots.task_variants import (
    ...
    sample_pass_object_randomization,
)
...
MODEL_NAME = "mixed_five_vla_100m"


def get_model_name(args) -> str:
    if args.model_backbone == "toy_cnn":
        return "mixed_five_vla_pass_object_mappo_pretrain"
    if is_mixed_five_vla_backbone(args.model_backbone):
        return "mixed_five_vla_100m_pass_object_mappo"
    raise ValueError(f"Unsupported model_backbone for this task: {args.model_backbone}")
...
def build_base_train_env_kwargs(args):
    return {
        "obs_mode": args.obs_mode,
        "control_mode": args.control_mode,
        "reward_mode": args.reward_mode,
...
def build_eval_env_kwargs(args):
    return {
        "obs_mode": args.obs_mode,
        "control_mode": args.control_mode,
        "reward_mode": args.reward_mode,
...
def build_train_env_spec_list(args):
    if getattr(args, "use_online_env_family_train_mix", False):
        return [dict(spec) for spec in ONLINE_FAMILY_TRAIN_ENV_KWARGS_LIST]
    return None


def sample_train_env_spec(args, train_env_spec_list, rng, train_env_spec_index):
    mode = getattr(args, "online_env_family_train_mix_mode", "random")
    if mode == "cycle":
        train_env_spec_index = (train_env_spec_index + 1) % len(train_env_spec_list)
        return dict(train_env_spec_list[train_env_spec_index]), train_env_spec_index

    clean_weight = max(float(getattr(args, "online_env_family_clean_weight", 0.2)), 0.0)
    family_weight = max(float(getattr(args, "online_env_family_variant_weight", 0.16)), 0.0)
    probs = np.asarray(
        [clean_weight] + [family_weight] * (len(train_env_spec_list) - 1),
        dtype=np.float64,
    )
    if float(probs.sum()) <= 0.0:
        probs = np.ones((len(train_env_spec_list),), dtype=np.float64)
    probs = probs / probs.sum()
    sampled_index = int(rng.choice(len(train_env_spec_list), p=probs))
    return dict(train_env_spec_list[sampled_index]), sampled_index


def apply_legacy_mixed_train_env_kwargs(args, env_kwargs):
    if not args.enable_mixed_train_envs:
        return env_kwargs, "mixed_train_disabled"
    variant_rng = np.random.default_rng(args.seed + 1729)
    level_probs = np.asarray(
        [
            args.mixed_env_clean_ratio,
            args.mixed_env_mild_ratio,
            args.mixed_env_hard_ratio,
        ],
        dtype=np.float64,
    )
    level_probs = np.clip(level_probs, 0.0, None)
    if float(level_probs.sum()) <= 0.0:
        level_probs = np.asarray([0.2, 0.5, 0.3], dtype=np.float64)
    ...
    random_env_kwargs, env_name = sample_pass_object_randomization(variant_rng, level=level)
    env_kwargs.update(random_env_kwargs)
    print(
        "[MixedEnv] Using pass-object geometry randomization split: "
        f"clear={args.mixed_env_clean_ratio}, "
        f"mild={args.mixed_env_mild_ratio}, "
        f"hard={args.mixed_env_hard_ratio}, "
        f"selected={env_name}"
    )
    return env_kwargs, env_name


def make_train_envs(args, env_kwargs, env_name):
    ...
    print(f"[TrainEnv] env={env_name} kwargs={env_kwargs}")
    return envs
...
def log_effective_transmission_stats(
    ...
    for client_name, client in clients.items():
        ...
        effective_ratio = float(matched_bytes) / float(total_bytes) if total_bytes > 0 else 0.0
...
def set_optimizer_phase_lrs(args, optimizer, critic_only: bool, train_actor_base: bool) -> None:
    for group in optimizer.param_groups:
        ...
        if group_name in {"vla_adapter_backbone", "smolvla_backbone", "efficientvla_backbone"}:
            group["lr"] = 0.0 if (critic_only or not train_actor_base) else args.backbone_learning_rate
        elif group_name in {"vla_adapter_state_projector", "smolvla_state_projector", "efficientvla_state_projector"}:
            group["lr"] = 0.0 if (critic_only or not train_actor_base) else args.state_learning_rate
        elif group_name in {"vla_adapter_heads", "smolvla_heads", "efficientvla_heads"}:
    ...
    if args.resume_dir is not None:
        ...
        if os.path.exists(resume_agent_path):
            ...
            try:
                ...
                print(
                    f"[Resume] failed to load resume checkpoint {resume_agent_path}: {exc}. "
                    "Fall back to init-agent-path / fresh init."
                )
            else:
                best_step = None
...
def build_clients(args, agent, infos, agent_names, device):
    if args.model_backbone == "toy_cnn":
        ...
        return {
            name: VLAClientForMultiAgent(
                name=name,
                large_model=agent,
                action_position_layer_prefix=f"action_position_placeholders.{name}",
                action_position_actor_layer_prefix=f"action_position_actor_placeholders.{name}",
                local_feature_dim=local_feature_dim,
                num_action_positions=int(infos["action_dims"][name]),
                device=device,
                local_action_dim=int(infos["action_dims"][name]),
                max_episode_steps=args.max_episode_steps,
    ...
    if is_mixed_five_vla_backbone(args.model_backbone):
        branch_specs = {
            agent.vla_agent_name: ("vla_actor", 3),
            agent.smolvla_agent_name: ("smolvla_actor", 1),
            agent.efficientvla_agent_name: ("efficientvla_actor", 1),
            agent.tinyvla_agent_name: ("tinyvla_actor", 1),
            agent.flower_agent_name: ("flower_actor", 1),
        }
        clients = {}
        for name in agent_names:
            actor_attr, feature_mult = branch_specs[name]
            actor = getattr(agent, actor_attr)
            clients[name] = VLAClientForMultiAgent(
                name=name,
                large_model=agent,
                action_position_layer_prefix=f"action_position_placeholders.{name}",
                action_position_actor_layer_prefix=f"action_position_actor_placeholders.{name}",
                local_feature_dim=int(actor.hidden_dim) * int(feature_mult),
                num_action_positions=int(infos["action_dims"][name]),
                device=device,
                local_action_dim=int(infos["action_dims"][name]),
                max_episode_steps=args.max_episode_steps,
    ...
    raise ValueError(f"Unsupported model_backbone for feature aggregation: {args.model_backbone}")
...
def main(args):
    ...
    if args.model_backbone == "toy_cnn":
        ...
        if getattr(args, "full_kl_coef", 0.0) > 0 or getattr(args, "log_full_kl", False):
            if accelerator.is_main_process:
                print("[Config] toy_cnn pass-object agent disables token KL; force full_kl_coef=0 and log_full_kl=False")
        ...
        head_trainable_parameters = (
            [p for p in agent.actor_heads.parameters() if p.requires_grad]
            + [p for p in agent.actor_logstd.parameters() if p.requires_grad]
        )
        encoder_trainable_parameters = (
            [p for p in agent.rgb_encoder.parameters() if p.requires_grad]
            + [p for p in agent.actor_state_encoders.parameters() if p.requires_grad]
        )
        critic_trainable_parameters = (
            [p for p in agent.critic_state_encoder.parameters() if p.requires_grad]
            + [p for p in agent.critic.parameters() if p.requires_grad]
        )
    elif is_mixed_five_vla_backbone(args.model_backbone):
        agent = MixedFiveVLA100MAgent(
            **infos,
            model_dir=args.model_dir,
            normalize_state=args.normalize_state,
        ...
        if getattr(args, "full_kl_coef", 0.0) > 0 or getattr(args, "log_full_kl", False):
            if accelerator.is_main_process:
                print("[Config] mixed_five_vla_100m disables full token-distribution KL; force full_kl_coef=0 and log_full_kl=False")
            args.full_kl_coef = 0.0
            args.log_full_kl = False
    else:
        raise ValueError(f"Unsupported model_backbone for this task: {args.model_backbone}")
    ...
    if args.model_backbone != "toy_cnn":
        base_param_groups = [
            ...
            {
                "params": [p for p in agent.tinyvla_actor.vla.parameters() if p.requires_grad],
                "lr": args.backbone_learning_rate,
                "group_name": "tinyvla_backbone",
            },
            {
                "params": [p for p in agent.tinyvla_actor.state_projector.parameters() if p.requires_grad],
                "lr": args.state_learning_rate,
                "group_name": "tinyvla_state_projector",
            },
            {
                "params": [p for p in agent.tinyvla_actor.context_projector.parameters() if p.requires_grad]
                + [p for p in agent.tinyvla_actor.action_mean_head.parameters() if p.requires_grad]
                + [p for p in agent.tinyvla_actor.log_std_head.parameters() if p.requires_grad],
                "lr": args.head_learning_rate,
                "group_name": "tinyvla_heads",
            },
            {
                "params": [p for p in agent.flower_actor.vla.parameters() if p.requires_grad],
                "lr": args.backbone_learning_rate,
                "group_name": "flower_backbone",
            },
            {
                "params": [p for p in agent.flower_actor.state_projector.parameters() if p.requires_grad],
                "lr": args.state_learning_rate,
                "group_name": "flower_state_projector",
            },
            {
                "params": [p for p in agent.flower_actor.context_projector.parameters() if p.requires_grad]
                + [p for p in agent.flower_actor.action_mean_head.parameters() if p.requires_grad]
                + [p for p in agent.flower_actor.log_std_head.parameters() if p.requires_grad],
                "lr": args.head_learning_rate,
                "group_name": "flower_heads",
            },
            {
    ...
    train_env_kwargs = build_base_train_env_kwargs(args)
    env_kwargs_for_eval = build_eval_env_kwargs(args)
    train_env_spec_list = build_train_env_spec_list(args)
    train_env_spec_index = 0
    train_env_switch_count = 0
    train_rollout_index = 0
    current_train_env_name = "Clean"
    train_env_family_rng = np.random.default_rng(args.seed + 20260711)
    if train_env_spec_list is None:
        train_env_kwargs, current_train_env_name = apply_legacy_mixed_train_env_kwargs(
            args, train_env_kwargs
        )
    ...
    if (
        not args.evaluate_mode
        and accelerator.is_main_process
        and global_steps >= int(step_infos["total_steps"])
    ):
        print(
            f"[Train] global_steps={global_steps} already reached total_steps={int(step_infos['total_steps'])}. "
            "Training will exit immediately. Increase --total-steps or clear --resume-dir."
        )

    agent, optimizer = accelerator.prepare(agent, optimizer)

    if train_env_spec_list is not None:
        if args.online_env_family_train_mix_mode == "cycle":
            current_train_env_spec = dict(train_env_spec_list[train_env_spec_index])
        else:
            current_train_env_spec, train_env_spec_index = sample_train_env_spec(
                args,
                train_env_spec_list,
                train_env_family_rng,
                train_env_spec_index - 1,
            )
        current_train_env_name = current_train_env_spec.pop("name")
        train_env_kwargs.update(current_train_env_spec)
        print(
            f"[TrainEnv] using online-rl family mix: "
            f"mode={args.online_env_family_train_mix_mode}, "
            f"rollouts_per_family={args.train_env_rollouts_per_family}, "
            f"family_count={len(train_env_spec_list)}"
        )
    envs = make_train_envs(args, train_env_kwargs, current_train_env_name)
    next_obs, _ = envs.reset(seed=args.seed + train_env_switch_count)
    ...
    while global_steps < step_infos["total_steps"]:
        if (
            train_env_spec_list is not None
            and train_rollout_index > 0
            and args.train_env_rollouts_per_family > 0
            and train_rollout_index % args.train_env_rollouts_per_family == 0
        ):
            envs.close()
            train_env_switch_count += 1
            train_env_kwargs = build_base_train_env_kwargs(args)
            current_train_env_spec, train_env_spec_index = sample_train_env_spec(
                args,
                train_env_spec_list,
                train_env_family_rng,
                train_env_spec_index,
            )
            current_train_env_name = current_train_env_spec.pop("name")
            train_env_kwargs.update(current_train_env_spec)
            envs = make_train_envs(args, train_env_kwargs, current_train_env_name)
        ...
        if accelerator.is_main_process:
            ...
            if "argmax_change_frac" in stats:
                writer.add_scalar(
                    "loss/argmax_change_frac",
                    stats["argmax_change_frac"],
                    global_steps,
                )
...
def parse_args():
    ...
    parser.add_argument("--ckpt-task-name", type=str, default="PassObjectFiveRobots-v1_ag")
    parser.add_argument("--seed", type=int, default=1788)
    parser.add_argument("--total-steps", type=int, default=20_000_000)
    parser.add_argument("--critic-warmup-rollouts", type=int, default=200)
    ...
    parser.add_argument(
        ...
        type=str,
        default="toy_cnn",
        choices=["toy_cnn", "mixed_five_vla_100m", "mixed_tiny_vla_smolvla_efficientvla"],
        help="Backbone used by pass-object feature-aggregator pretrain.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="cycle train envs across the same clean + online_rl environment families used by continual online adaptation",
    )
    parser.add_argument(
        "--train-env-rollouts-per-family",
        type=int,
        default=4,
        help="number of rollouts to train on one online-style train env family before switching to the next",
    )
    parser.add_argument(
        "--online-env-family-train-mix-mode",
        type=str,
        default="random",
        choices=["random", "cycle"],
        help="random mixes online-style train env families by weight; cycle iterates through them in order",
    )
    parser.add_argument(
        "--online-env-family-clean-weight",
        type=float,
        default=0.2,
        help="sampling weight for the clean env when online-style train env family mixing is enabled",
    )
    parser.add_argument(
        "--online-env-family-variant-weight",
        type=float,
        default=0.16,
        help="sampling weight for each non-clean online-style env family when mixed sampling is enabled",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--mixed-env-mild-size-min",
        type=float,
        default=0.0,
        help="deprecated for pass-object; mild geometry mixing now samples lane/yaw/robot poses directly",
    )
    parser.add_argument(
        "--mixed-env-mild-size-max",
        type=float,
        default=0.0,
        help="deprecated for pass-object; mild geometry mixing now samples lane/yaw/robot poses directly",
    )
    parser.add_argument(
        "--mixed-env-hard-size-min",
        type=float,
        default=0.0,
        help="deprecated for pass-object; hard geometry mixing now samples lane/yaw/robot poses directly",
    )
    parser.add_argument(
        "--mixed-env-hard-size-max",
        type=float,
        default=0.0,
        help="deprecated for pass-object; hard geometry mixing now samples lane/yaw/robot poses directly",
    )
