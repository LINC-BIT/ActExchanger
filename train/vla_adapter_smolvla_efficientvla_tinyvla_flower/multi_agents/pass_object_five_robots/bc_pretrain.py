...
from mani_skill.utils.wrappers.record import RecordEpisode, Step
...
from train.toy_cnn.multi_agents.pass_object.mappo_pretrain import (
...
from train.vla_adapter_smolvla_efficientvla_tinyvla_flower.multi_agents.pass_object_five_robots.task_variants import (
    build_pass_object_toy_cnn_agent_obs_rules,
...
MODEL_NAME = "mixed_five_vla_pass_object_mappo_pretrain"
ALGO_NAME = "sft"


def _repeat_batched_action(action, num_envs: int):
    if isinstance(action, dict):
        return {k: _repeat_batched_action(v, num_envs) for k, v in action.items()}
    if isinstance(action, torch.Tensor):
        if action.shape[0] == num_envs:
            return action
        if action.shape[0] == 1:
            return action.repeat(num_envs, *([1] * (action.ndim - 1)))
        raise ValueError(f"Unexpected action batch shape {tuple(action.shape)} for num_envs={num_envs}")
    if isinstance(action, np.ndarray):
        if action.shape[0] == num_envs:
            return action
        if action.shape[0] == 1:
            return np.repeat(action, num_envs, axis=0)
        raise ValueError(f"Unexpected action batch shape {action.shape} for num_envs={num_envs}")
    return action


class DictActionRecordEpisode(RecordEpisode):
    def reset(
        self,
        *args,
        seed=None,
        ...
        **kwargs,
        ...
        if self.save_trajectory:
            ...
            action = _repeat_batched_action(action, self.num_envs)
            ...
            first_step = Step(
                ...
                reward=np.zeros((1, self.num_envs), dtype=float),
            ...
            if full_reset or self._trajectory_buffer is None or len(self._trajectory_buffer.done) == 0:
    ...
    def flush_trajectory(
        ...
        for env_idx in env_idxs_to_flush:
            ...
            if save:
                ...
                def recursive_add_to_h5py(group, data, key, already_sliced=False):
                    if isinstance(data, dict):
                        subgrp = group.create_group(key, track_order=True)
                        for k in data.keys():
                            recursive_add_to_h5py(subgrp, data[k], k, already_sliced=already_sliced)
                    else:
                        if already_sliced:
                ...
                if isinstance(self._trajectory_buffer.action, dict):
                    recursive_add_to_h5py(group, actions, "actions", already_sliced=True)
                else:
...
def build_toy_cnn_agent_obs_rules(agent_names: Sequence[str]) -> Dict[str, list[str]]:
    return build_pass_object_toy_cnn_agent_obs_rules(agent_names)
...
def get_agent_info(args) -> Dict[str, Any]:
    ...
    info = {
        ...
        "action_dims": {name: env.single_action_space[name].shape[0] for name in agent_names},
    }
    env.close()
    return info
...
def resolve_toy_cnn_expert_candidates(args) -> list[str]:
    ...
    expert_root = os.path.join(
        args.save_dir,
        f"{args.task_name}/ppo/{args.robot_name}/mixed_five_vla_pass_object_mappo_pretrain",
    )
    candidates = []
    ...
    if not candidates:
        raise FileNotFoundError(
            "No toy_cnn PassObjectFiveRobots expert checkpoint found. "
...
def load_toy_cnn_expert_state(args, expert: ToyCNNMAPPOAgent, expected_agent_names: Sequence[str]) -> str:
    ...
    for path in candidates:
        ...
        try:
            ...
            if args.expert_agent_path:
                raise RuntimeError(
                    "Incompatible toy_cnn expert checkpoint for PassObjectFiveRobots collection.\n"
                    + "\n".join(errors)
                    + "\nUse a PassObjectFiveRobots-v1 toy_cnn checkpoint trained with the current env/obs keys."
                ) from exc

    raise RuntimeError(
        "No compatible toy_cnn PassObjectFiveRobots expert checkpoint found.\n"
        + "\n".join(errors[-8:])
        + "\nSet EXPERT_AGENT_PATH to a compatible "
        "ckpt/PassObjectFiveRobots-v1/ppo/panda_so100_widowx_xarm6_inspire/mixed_five_vla_pass_object_mappo_pretrain/<run>/best_agent.pt."
    )
...
def remove_trajectory_files(traj_path: str) -> None:
    stale_paths = {traj_path, traj_path.replace(".h5", ".json")}
    legacy_base = os.path.join(os.path.dirname(traj_path), Path(traj_path).stem)
    stale_paths.add(f"{legacy_base}.h5")
    stale_paths.add(f"{legacy_base}.json")
    for stale_path in stale_paths:
        if os.path.exists(stale_path):
            os.remove(stale_path)


def trajectory_record_name(traj_path: str) -> str:
    filename = Path(traj_path).name
    return filename[:-3] if filename.endswith(".h5") else filename


def episode_succeeded(episode: Dict[str, Any]) -> bool:
    return bool(episode.get("success_once", episode.get("success", False)))


def infos_success_tensor(infos: Dict[str, Any]):
    if "success_once" in infos:
        return infos["success_once"]
...
def validate_trajectory_dataset(traj_path: str, require_success: bool = True) -> tuple[bool, str]:
    ...
    try:
        ...
        if require_success:
            episodes = [episode for episode in episodes if episode_succeeded(episode)]
        if not episodes:
...
def collect_expert_trajectories(args, ckpt: Dict[str, str]) -> str:
    ...
    num_collect_envs = 1 if args.replay_friendly_collect else args.num_collect_envs
    base_env = gym.make(args.task_name, num_envs=num_collect_envs, **env_kwargs)
    record_env = DictActionRecordEpisode(
        base_env,
        output_dir=output_dir,
        save_trajectory=True,
        trajectory_name=trajectory_record_name(traj_path),
        save_video=False,
        record_reward=False,
        record_env_state=args.record_env_state or args.replay_friendly_collect,
        source_type="rl",
    ...
    while successful_episodes < args.num_successful_trajectories:
        ...
        if done.any():
            ...
            step_success = infos_success_tensor(infos)[done_env_idx].detach().cpu().numpy().astype(bool)
...
def build_agent(args, infos: Dict[str, Any], device: torch.device):
    agent = PassObjectToyCNNAgent(**infos, normalize_state=args.normalize_state)
    return agent.to(device)


def build_sft_optimizer(args, agent: PassObjectToyCNNAgent) -> torch.optim.Optimizer:
...
def resolve_trajectory_path(args, ckpt: Dict[str, str]) -> str:
    ...
    if args.collect_only:
        raise RuntimeError(
            "PassObjectFiveRobots BC does not collect expert trajectories yet. "
...
def filter_successful_trajectories(traj_path: str, max_episodes: int | None = None) -> int:
    ...
    successful_episodes = [episode for episode in meta["episodes"] if episode_succeeded(episode)]
...
def _load_actions(
    ...
    if isinstance(action_node, h5py.Dataset):
        ...
        total_action_dim = int(sum(int(action_dims[name]) for name in agent_names))
        if flat_actions.shape[-1] != total_action_dim:
            raise ValueError(
                f"Flat action dim mismatch: got {flat_actions.shape[-1]}, expected {total_action_dim}"
            )
        start = 0
        outputs = {}
        for name in agent_names:
            dim = int(action_dims[name])
            outputs[name] = flat_actions[:, start : start + dim]
            start += dim
        return outputs
...
def load_sft_dataset_from_trajectories(
    ...
    if cache_path and os.path.exists(cache_path) and not args.force_rebuild_dataset_cache:
    ...
    with open(json_path, "r") as f:
        meta = json.load(f)
    success_episodes = [episode for episode in meta["episodes"] if episode_succeeded(episode)]
    ...
    with h5py.File(trajectory_h5_path, "r") as h5_file:
        for episode in tqdm(success_episodes, ascii=True, desc="Build SFT Dataset"):
            ...
            actions = _load_actions(traj, horizon, agent_names, action_dims)
            ...
            global_states.append(torch.as_tensor(parsed["global_state"]).to(dtype=torch.float16, device="cpu"))
            trajectory_lengths.append(horizon)
            for name in agent_names:
                per_agent_states[name].append(
                    torch.as_tensor(parsed[f"agent_states_{name}"]).to(dtype=torch.float16, device="cpu")
                )
                per_agent_actions[name].append(
                    torch.as_tensor(actions[name]).to(dtype=torch.float16, device="cpu")
                )
    ...
    dataset = {
        ...
        "action_dims": {name: int(action_dims[name]) for name in agent_names},
    ...
    if args.save_dataset_cache and cache_path:
        ...
        print("[Dataset] skip dataset cache save")
    return dataset
...
def train_sft(args) -> None:
    ...
    agent_names = infos["agent_names"]
    dataset = load_sft_dataset_from_trajectories(
        args,
        ckpt,
        trajectory_h5_path,
        agent_names,
    ...
    while global_updates < total_iters:
        ...
        with torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=autocast_enabled):
            ...
            log_prob_terms = [
                log_probs[name] / max(1, int(agent.action_dims[name]))
                for name in agent_names
            ]
            entropy_terms = [
                entropies[name] / max(1, int(agent.action_dims[name]))
                for name in agent_names
            ]
            mean_log_prob = torch.stack(log_prob_terms, dim=0).mean()
            mean_entropy = torch.stack(entropy_terms, dim=0).mean()
            entropy_per_dim = mean_entropy
            bc_loss = -mean_log_prob
...
def parse_args():
    ...
    parser.add_argument("--save-dataset-cache", action=argparse.BooleanOptionalAction, default=False)
    ...
    parser.add_argument("--record-env-state", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--replay-friendly-collect", action=argparse.BooleanOptionalAction, default=False)
