...
MixedFiveVLA100MAgent = PassObjectToyCNNAgent
MixedTinyVLAAdapterSmolVLAEfficientVLAAgent = PassObjectToyCNNAgent
build_mixed_mappo_optimizer = build_toy_cnn_mappo_optimizer
...
ALGO_NAME = "ppo"
AGENT_CLS = PassObjectToyCNNAgent
OPTIMIZER_FN = build_toy_cnn_mappo_optimizer
...
def set_optimizer_phase_lrs(args, optimizer, critic_only: bool, actor_lr_scale: float = 1.0) -> None:
    actor_lr_scale = float(np.clip(actor_lr_scale, 0.0, 1.0))
    for group in optimizer.param_groups:
        ...
        if group_name in {
            ...
            group["lr"] = 0.0 if critic_only else args.backbone_learning_rate * actor_lr_scale
        elif group_name in {
            ...
            group["lr"] = 0.0 if critic_only else args.state_learning_rate * actor_lr_scale
        elif group_name in {
            "vla_adapter_heads",
            "smolvla_heads",
            "efficientvla_heads",
            ...
            group["lr"] = 0.0 if critic_only else args.head_learning_rate * actor_lr_scale
        elif group_name in {"critic_state_encoder", "critic_visual_encoder", "critic"}:
            group["lr"] = args.value_head_learning_rate


def get_actor_lr_scale(args, global_steps: int, step_infos) -> float:
    critic_warmup_steps = int(step_infos["critic_warmup_steps"])
    if int(global_steps) < critic_warmup_steps:
        return 0.0
    actor_warmup_steps = int(step_infos["rollot_steps"]) * int(getattr(args, "actor_warmup_rollouts", 0))
    if actor_warmup_steps <= 0:
        return 1.0
    progressed_steps = int(global_steps) - critic_warmup_steps
    if progressed_steps <= 0:
        return 0.0
    return min(float(progressed_steps) / float(actor_warmup_steps), 1.0)
...
def main(args):
    ...
    if accelerator.is_main_process:
        ...
        print(
        f"[Model] backbone=toy_cnn total_params={total_params / 1e6:.2f}M "
    ...
    actor_lr_scale = get_actor_lr_scale(args, global_steps, step_infos)
    set_optimizer_phase_lrs(args, optimizer, critic_only=critic_only, actor_lr_scale=actor_lr_scale)
    ...
    while global_steps < step_infos["total_steps"]:
        ...
        if phase != current_phase:
            ...
            print(
                ...
                f"actor_warmup_rollouts={args.actor_warmup_rollouts})"
            )
        ...
        if accelerator.is_main_process and writer is not None:
            writer.add_scalar("training/actor_lr_scale", actor_lr_scale, global_steps)
        ...
        if accelerator.is_main_process:
            y_pred = val_buf.flatten(0, 1).detach().cpu().numpy().astype(np.float64, copy=False)
            y_true = ret_buf.flatten(0, 1).detach().cpu().numpy().astype(np.float64, copy=False)
            finite_mask = np.isfinite(y_pred) & np.isfinite(y_true)
            if finite_mask.any():
                y_pred = y_pred[finite_mask]
                y_true = y_true[finite_mask]
                var_y = np.var(y_true, dtype=np.float64)
                explained_var = np.nan if (not np.isfinite(var_y) or var_y <= 0.0) else 1 - np.var(
                    y_true - y_pred,
                    dtype=np.float64,
                ) / var_y
            else:
...
def parse_args():
    ...
    parser.add_argument("--actor-warmup-rollouts", type=int, default=25)
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional task name used only for checkpoint directory layout.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="toy_cnn",
        choices=["toy_cnn", "mixed_tiny_vla_smolvla_efficientvla", "mixed_five_vla_100m"],
        help="Use the toy_cnn pass-object backbone.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Frozen low-level policy checkpoint used by planner-style baselines.",
    )
    ...
    parser.add_argument(
        "--planner-low-level-deterministic",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Use deterministic low-level actions under the high-level planner.",
    )
    parser.add_argument("--maporl-team-adv-coef", type=float, default=0.0)
    parser.add_argument("--maporl-kl-coef", type=float, default=0.05)
    ...
    parser.add_argument("--magrpo-kl-coef", type=float, default=0.05)
    ...
    parser.add_argument("--mpdf-kl-coef", type=float, default=0.05)
