...
from train.vla_adapter_smolvla_efficientvla_tinyvla_flower.multi_agents.pass_object_five_robots.online_utils import (
...
def make_envs_for_env_kwargs(
    args,
    ckpt,
    train_env_kwargs_update_id,
    base_env_kwargs,
    env_kwargs_list,
    eval_env_kwargs_update_id=None,
):
    if eval_env_kwargs_update_id is None:
        eval_env_kwargs_update_id = train_env_kwargs_update_id

    train_env_kwargs = base_env_kwargs.copy()
    train_env_kwargs_update = env_kwargs_list[train_env_kwargs_update_id].copy()
    train_env_name = train_env_kwargs_update.pop("name")
    train_env_kwargs.update(**train_env_kwargs_update)

    eval_env_kwargs = base_env_kwargs.copy()
    eval_env_kwargs_update = env_kwargs_list[eval_env_kwargs_update_id].copy()
    eval_env_name = eval_env_kwargs_update.pop("name")
    eval_env_kwargs.update(**eval_env_kwargs_update)

    env_kwargs_for_eval = eval_env_kwargs.copy()
    ...
    eval_envs = make_eval_envs(
        ...
        video_dir=os.path.join(eval_video_root, eval_env_name),
    )
    ...
    return envs, eval_envs, train_env_name, eval_env_name
...
def run_online_training(
    ...
    *,
    ...
    env_kwargs_list = DEFAULT_ONLINE_ENV_KWARGS_LIST if env_kwargs_list is None else env_kwargs_list
    ...
    current_env_index = 0
    train_env_mix_rng = random.Random(args.seed + 7919)
    current_train_env_index = 0

    def resolve_train_env_index(eval_env_index: int) -> int:
        if not getattr(args, "mix_train_online_envs", False):
            return eval_env_index
        return train_env_mix_rng.randrange(len(env_kwargs_list))

    current_train_env_index = resolve_train_env_index(current_env_index)
    training_start_time = time.monotonic()
    envs, eval_envs, current_train_env_name, current_env_name = make_envs_for_env_kwargs(
        args,
        ckpt,
        current_train_env_index,
        base_env_kwargs,
        env_kwargs_list,
        eval_env_kwargs_update_id=current_env_index,
    )
    next_obs, _ = envs.reset(seed=args.seed)
    next_done = torch.zeros(args.num_envs, device=device)

    def maybe_switch_envs():
        nonlocal envs, eval_envs, next_obs, next_done
        nonlocal current_env_index, current_env_name
        nonlocal current_train_env_index, current_train_env_name
        ...
        previous_train_env_name = current_train_env_name
        ...
        next_obs, _ = envs.reset(seed=args.seed)
        next_done = torch.zeros(args.num_envs, device=device)
        if getattr(args, "mix_train_online_envs", False):
            print(
                f"[OnlineRL] switch eval env from {previous_env_name} to {current_env_name}, "
                f"train env from {previous_train_env_name} to {current_train_env_name} "
    ...
    while global_steps < step_infos["total_steps"]:
        ...
        if writer is not None and elapsed_minutes is not None:
            ...
            writer.add_scalar("continual/current_train_env_index", current_train_env_index, global_steps)
        ...
        if switched:
            if getattr(args, "mix_train_online_envs", False):
                print(f"[OnlineRL] active eval env={current_env_name}, train env={current_train_env_name}")
            else:
        ...
        if rollout_mode == "maple":
            ...
            if len(rollout) == 10 and isinstance(rollout[-1], Mapping):
                (
                    obs_buf,
                    action_buf,
                    logp_buf,
        ...
        if clients is not None:
            ...
            if accelerator.is_main_process and writer is not None:
                ...
                log_effective_transmission_stats(writer, clients, global_steps)
