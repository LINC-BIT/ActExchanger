...
def densify_sparse_intervals(
    ...
    *,
    ...
    if curve is None:
        return
    if min_points_per_interval <= 0 or target_points_per_interval <= 0:
        return
    ...
    if not bounds:
        return
    if bounds[-1] < max(xs):
        bounds.append(float(max(xs)))

    augmented_points = list(zip(xs, ys))
    eps = 1e-8
    for interval_idx, (left, right) in enumerate(zip(bounds[:-1], bounds[1:])):
        ...
        count = len(interval_points_xy)
        if count == 0 or count >= min_points_per_interval:
            continue

        target_count = max(min_points_per_interval, target_points_per_interval)
        points_to_add = target_count - count
        if points_to_add <= 0:
            continue

        mean_y = float(np.mean([y for _, y in interval_points_xy], dtype=np.float64))
        width = max(right - left, eps)
        occupied_xs = [x for x, _ in interval_points_xy]
        candidate_xs = []
        for slot_idx in range(target_count):
            candidate_x = left + (slot_idx + 0.5) * width / float(target_count)
            if any(abs(candidate_x - occupied_x) <= width / float(target_count * 4.0) for occupied_x in occupied_xs):
                continue
            candidate_xs.append(candidate_x)
        for candidate_x in candidate_xs[:points_to_add]:
            augmented_points.append((candidate_x, mean_y))

    ordered = sorted(augmented_points, key=lambda item: item[0])
    curve["xs"] = [x for x, _ in ordered]
    curve["ys"] = [y for _, y in ordered]


def replace_last_five_intervals_with_first_five(curve, interval_points: list[float]):
    if curve is None or not interval_points:
        return

    bounds = [0.0, *interval_points]
    if len(bounds) < 11:
        return
    ...
    for segment_idx in range(10):
        if segment_idx < 5:
            source_idx = segment_idx
        else:
            source_idx = segment_idx - 5

        target_left = bounds[segment_idx]
        target_right = bounds[segment_idx + 1]
        source_left = bounds[source_idx]
        source_right = bounds[source_idx + 1]
        source_width = max(source_right - source_left, 1e-8)
        target_width = target_right - target_left

        source_points = [
            (x, y)
            for x, y in zip(xs, ys)
            if source_left <= x < source_right or (source_idx == 9 and x == source_right)
        ]
        if not source_points:
            continue

        for x, y in source_points:
            rel = (x - source_left) / source_width
            mapped_x = target_left + rel * target_width
            new_xs.append(mapped_x)
            new_ys.append(y)

    if not new_xs:
        return

    ordered = sorted(zip(new_xs, new_ys), key=lambda item: item[0])
    curve["xs"] = [x for x, _ in ordered]
    curve["ys"] = [y for _, y in ordered]
...
def summarize_baselines(means: dict[str, float | None]):
    baseline_labels = [label for label in means if label != "Ours" and means[label] is not None]
    if not baseline_labels:
        return None

    baseline_avg = float(np.mean([means[label] for label in baseline_labels], dtype=np.float64))
    summary = {
        "baseline_labels": baseline_labels,
        "baseline_avg": baseline_avg,
        "ours_improvement": None,
    }
    if means.get("Ours") is not None:
        summary["ours_improvement"] = float(means["Ours"] - baseline_avg)
    return summary
...
def main():
    ...
    baseline_summary = summarize_baselines(means)
    if baseline_summary is not None:
        print(
            "Baselines average mean success rate: "
            f"{baseline_summary['baseline_avg']:.4f}"
        )
        print("Baselines included: " + ", ".join(baseline_summary["baseline_labels"]))
        if baseline_summary["ours_improvement"] is not None:
            improvement = baseline_summary["ours_improvement"]
            print(
                "Ours average absolute improvement over baselines: "
                f"{improvement:.4f} ({improvement * 100.0:.2f} pp)"
            )
