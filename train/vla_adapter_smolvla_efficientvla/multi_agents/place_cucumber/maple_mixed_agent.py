...
class ThreeVLAMapleAdapterAgent(MixedTinyVLAAdapterSmolVLAEfficientVLAAgent):
    def __init__(
        self,
        *args,
        latent_layers: int = 2,
        ...
        self.agent_feature_projectors = nn.ModuleDict(
            {
                ...
                self.efficientvla_agent_name: nn.Sequential(
                    nn.LayerNorm(self.efficientvla_actor.hidden_dim),
                    nn.Linear(self.efficientvla_actor.hidden_dim, hidden_dim),
                ),
            }
        ...
        self.agent_delta_heads = nn.ModuleDict(
            {
                ...
                self.efficientvla_agent_name: nn.Sequential(
                    nn.LayerNorm(hidden_dim),
                    nn.Linear(hidden_dim, self.efficientvla_actor.hidden_dim),
                ),
            }
        ).to(dtype=torch.float32)
    ...
    def _adapt_actor_state_features(
        ...
        shared_tokens = torch.stack(
            [
                ...
                self.agent_feature_projectors[self.efficientvla_agent_name](efficientvla_state_feature),
            ],
            dim=1,
        )
        ...
        delta_efficient = self.agent_delta_heads[self.efficientvla_agent_name](coordinated[:, 2, :])
    ...
    def get_action_and_value(
        ...
        try:
            result = super().get_action_and_value(
                ...
                return_action_bins=return_action_bins,
        ...
        if return_token_logits and not return_action_bins:
            actions_out, log_probs, entropies, value, token_logits = result
        elif return_action_bins:
            actions_out, log_probs, entropies, value, action_bins_out = result
        else:
...
def build_optimizer(args, agent: ThreeVLAMapleAdapterAgent) -> torch.optim.Optimizer:
    maple_param_groups = [
        {
    ...
    core_trainable = any(
        ...
        for module in [
            ...
            agent.efficientvla_actor.vla,
            agent.efficientvla_actor.state_projector,
            agent.efficientvla_actor.context_projector,
            agent.efficientvla_actor.action_mean_head,
            agent.efficientvla_actor.log_std_head,
