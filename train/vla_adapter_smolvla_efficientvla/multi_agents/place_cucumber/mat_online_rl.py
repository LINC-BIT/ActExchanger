...
class ThreeVLAMATOnlineAgent(ThreeVLAMATAdapterAgent):
...
def build_optimizer(args, agent):
    ...
    if adapter_params:
        optimizer.add_param_group({"params": adapter_params, "lr": args.head_learning_rate, "group_name": "mat_adapter"})
    return optimizer


mappo_online_rl.AGENT_CLS = ThreeVLAMATOnlineAgent
mappo_online_rl.OPTIMIZER_FN = build_optimizer
mappo_online_rl.UPDATE_FN = mat_update_on_policy
mappo_online_rl.ALGO_NAME = "mat"
mappo_online_rl.MODEL_NAME = "vla_adapter_smolvla_efficientvla_mat"
