...
def get_model_name(args) -> str:
    if args.model_backbone == "mixed_tiny_vla_smolvla_efficientvla":
        if ALGO_NAME == "ppo" and MODEL_NAME == "vla_adapter_smolvla_efficientvla":
...
def main(args):
    ...
    if getattr(args, "full_kl_coef", 0.0) > 0 or getattr(args, "log_full_kl", False):
        if accelerator.is_main_process:
            print("[Config] mixed continuous branches disable full token-distribution KL; force full_kl_coef=0 and log_full_kl=False")
