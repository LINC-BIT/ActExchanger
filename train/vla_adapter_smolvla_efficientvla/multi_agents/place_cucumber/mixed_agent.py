...
class MixedTinyVLAAdapterSmolVLAEfficientVLAAgent(nn.Module):
    """Three-architecture mixed agent: VLA-Adapter tiny, SmolVLA tiny, EfficientVLA tiny."""
    ...
    def __init__(
        ...
        if len(agent_names) != 3:
            raise ValueError(
                "MixedTinyVLAAdapterSmolVLAEfficientVLAAgent expects exactly "
                f"3 agents [vla_adapter, smolvla, efficientvla], got {agent_names}"
            )
        ...
        self.efficientvla_agent_name = self.agent_names[2]
        self.vla_state_dim = self.state_dims[self.vla_agent_name]
        self.smolvla_state_dim = self.state_dims[self.smolvla_agent_name]
        self.efficientvla_state_dim = self.state_dims[self.efficientvla_agent_name]
        ...
        self.smolvla_actor = TinyContinuousVLAActor(
            state_dim=self.smolvla_state_dim,
        ...
        self.efficientvla_actor = TinyContinuousVLAActor(
            state_dim=self.efficientvla_state_dim,
            action_dim=self.action_dim,
            image_size=resolved_image_size,
        ...
        self.efficientvla_actor_action_position_placeholders = nn.ModuleList(
            [nn.Identity() for _ in range(self.action_dim)]
        )
        self.efficientvla_actor_action_position_actor_placeholders = nn.ModuleList(
            [nn.Identity() for _ in range(self.action_dim)]
        )
        ...
        critic_visual_input_dim = int(tiny_hidden_dim) + int(smolvla_hidden_dim) + int(efficientvla_hidden_dim)
    ...
    def _vla_action_bins_input(self, action_bins) -> torch.Tensor:
        ...
        if is_float:
            ...
            return self.vla_actor.env_actions_to_bin_indices(action_tensor).to(
                device=self.device,
                dtype=torch.long,
            )
    ...
    def _adapt_actor_state_features(
        ...
        return vla_state_feature, smolvla_state_feature, efficientvla_state_feature
    ...
    def _compute_value(self, batch: Dict[str, Any]) -> torch.Tensor:
        ...
        rgb = batch["rgb"]
        with torch.no_grad():
            vla_visual = self.vla_actor._project_vision_features(
                self.vla_actor._prepare_pixel_values(rgb)
            ).mean(dim=1).to(torch.float32)
            smol_visual = self.smolvla_actor.encode_visual_feature(rgb)
            efficient_visual = self.efficientvla_actor.encode_visual_feature(rgb)
        visual_feature = self.critic_visual_encoder(
            torch.cat([vla_visual, smol_visual, efficient_visual], dim=-1)
        )
    ...
    def get_action_and_value(
        ...
        **kwargs,
        ...
        rgb = batch["rgb"]
        vla_state = self._agent_state(batch, self.vla_agent_name)
        smol_state = self._agent_state(batch, self.smolvla_agent_name)
        efficient_state = self._agent_state(batch, self.efficientvla_agent_name)
        ...
        efficient_planner_subtasks = (
            None if planner_subtasks is None else planner_subtasks[self.efficientvla_agent_name]
        )
        ...
        efficient_state_feature = self.efficientvla_actor.state_projector(efficient_state)
        vla_state_feature, smol_state_feature, efficient_state_feature = self._adapt_actor_state_features(
            vla_state_feature,
            smol_state_feature,
            efficient_state_feature,
        )
        ...
        smol_out = self.smolvla_actor.sample_actions(
            ...
            actions_input=self._continuous_actions_input_from_any(
                actions_input,
                action_bins_input,
        ...
        efficient_out = self.efficientvla_actor.sample_actions(
            rgb,
            efficient_state,
            state_features=efficient_state_feature,
            actions_input=self._continuous_actions_input_from_any(
                actions_input,
                action_bins_input,
            ...
            deterministic=deterministic,
            action_position_placeholders=self.efficientvla_actor_action_position_placeholders,
            action_position_actor_placeholders=self.efficientvla_actor_action_position_actor_placeholders,
            planner_subtasks=efficient_planner_subtasks,
        )
        actions_out = {
            ...
            self.efficientvla_agent_name: efficient_out["actions"].detach().to(dtype=torch.float32).cpu().numpy(),
        }
        log_probs = {
            self.vla_agent_name: vla_out["log_prob"],
            self.smolvla_agent_name: smol_out["log_prob"],
            self.efficientvla_agent_name: efficient_out["log_prob"],
        }
        entropies = {
            self.vla_agent_name: vla_out["entropy"],
            self.smolvla_agent_name: smol_out["entropy"],
            self.efficientvla_agent_name: efficient_out["entropy"],
        }
        ...
        if return_action_bins:
            ...
            if self.smolvla_agent_name not in action_bins_out:
                action_bins_out[self.smolvla_agent_name] = self._continuous_actions_to_bin_indices(
                    actions_out[self.smolvla_agent_name]
                )
            if self.efficientvla_agent_name not in action_bins_out:
                action_bins_out[self.efficientvla_agent_name] = self._continuous_actions_to_bin_indices(
                    actions_out[self.efficientvla_agent_name]
                )
    ...
    def load_checkpoint_state_dict(self, state_dict):
        ...
        for key, value in state_dict.items():
            ...
            matched[key] = value
        if not matched:
            raise RuntimeError("No compatible parameters found for mixed tiny VLA agent")
        ...
        print(
            "[Checkpoint] MixedTinyVLAAdapterSmolVLAEfficientVLAAgent "
...
MixedAgent = MixedTinyVLAAdapterSmolVLAEfficientVLAAgent
