...
MODEL_NAME = "vla_adapter_smolvla_efficientvla"
...
def get_model_name(args) -> str:
    if args.model_backbone == "mixed_tiny_vla_smolvla_efficientvla":
        return "vla_adapter_smolvla_efficientvla_mappo"
...
def apply_legacy_mixed_train_env_kwargs(args, env_kwargs):
    ...
    size_rng = np.random.default_rng(args.seed + 1729)
    size_probs = np.asarray(
        [
    ...
    size_probs = np.clip(size_probs, 0.0, None)
    if float(size_probs.sum()) <= 0.0:
        size_probs = np.asarray([0.2, 0.5, 0.3], dtype=np.float64)
    ...
    mild_size_min = min(float(args.mixed_env_mild_size_min), float(args.mixed_env_mild_size_max))
    mild_size_max = max(float(args.mixed_env_mild_size_min), float(args.mixed_env_mild_size_max))
    hard_size_min = min(float(args.mixed_env_hard_size_min), float(args.mixed_env_hard_size_max))
    hard_size_max = max(float(args.mixed_env_hard_size_min), float(args.mixed_env_hard_size_max))
    if size_level == "clear":
        cucumber_size_scale = 1.0
    elif size_level == "mild":
        cucumber_size_scale = float(size_rng.uniform(mild_size_min, mild_size_max))
    else:
        cucumber_size_scale = float(size_rng.uniform(hard_size_min, hard_size_max))
    env_kwargs.update(
        {
            ...
            "cucumber_size_scale": cucumber_size_scale,
        }
    )
    print(
        ...
        f"size_level={size_level}, cucumber_size_scale={cucumber_size_scale:.3f}, "
        f"mild_size_range=({mild_size_min:.3f}, {mild_size_max:.3f}), "
        f"hard_size_range=({hard_size_min:.3f}, {hard_size_max:.3f})"
    )
    return env_kwargs, f"legacy_mixed_{size_level}"
...
def build_clients(args, agent, infos, agent_names, device):
    if args.model_backbone == "mixed_tiny_vla_smolvla_efficientvla":
        return {
            ...
            agent.efficientvla_agent_name: VLAClientForMultiAgent(
                name=agent.efficientvla_agent_name,
                large_model=agent,
                action_position_layer_prefix="efficientvla_actor_action_position_placeholders",
                action_position_actor_layer_prefix="efficientvla_actor_action_position_actor_placeholders",
                local_feature_dim=agent.efficientvla_actor.hidden_dim,
                num_action_positions=infos["action_dim"],
                device=device,
                local_action_dim=infos["action_dim"],
                max_episode_steps=args.max_episode_steps,
...
def main(args):
    ...
    if args.model_backbone == "mixed_tiny_vla_smolvla_efficientvla":
        ...
        if getattr(args, "full_kl_coef", 0.0) > 0 or getattr(args, "log_full_kl", False):
            if accelerator.is_main_process:
                print("[Config] mixed_tiny_vla_smolvla_efficientvla disables full token-distribution KL; force full_kl_coef=0 and log_full_kl=False")
    ...
    for group in base_param_groups:
        ...
        if group_name in {"vla_adapter_heads", "smolvla_heads", "efficientvla_heads"}:
...
def parse_args():
    ...
    parser.add_argument("--ckpt-task-name", type=str, default="PlaceCucumber-v1_ag")
    ...
    parser.add_argument(
        ...
        help="Use the mixed tiny VLA-Adapter + SmolVLA + EfficientVLA PPO backbone.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.70,
        help="minimum cucumber_size_scale for random mild train-env geometry",
    )
    parser.add_argument(
        "--mixed-env-mild-size-max",
        type=float,
        default=1.25,
        help="maximum cucumber_size_scale for random mild train-env geometry",
    )
    parser.add_argument(
        "--mixed-env-hard-size-min",
        type=float,
        default=0.50,
        help="minimum cucumber_size_scale for random hard train-env geometry",
    )
    parser.add_argument(
        "--mixed-env-hard-size-max",
        type=float,
        default=0.70,
        help="maximum cucumber_size_scale for random hard train-env geometry",
    )
    parser.add_argument(
