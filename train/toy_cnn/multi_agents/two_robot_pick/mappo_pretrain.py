...
def make_collate_fn(args, cameras, device):
    ...
    def collate_fn(obs):
        if isinstance(obs['rgb'], np.ndarray):
            ...
            agent_states = {}
            for agent_name, agent_state in obs['agent_states'].items():
                agent_states[agent_name] = torch.from_numpy(agent_state)
        ...
        for agent_name in agent_states:
            agent_states[agent_name] = agent_states[agent_name].to(device)
...
def get_agent_info(args, env_kwargs, agent_obs_rules, collate_fn):
    ...
    state_dim = batch['agent_states'][agent_names[0]].shape[-1]
    ...
    for agent_name in agent_names:
        if batch['agent_states'][agent_name].shape[-1] != state_dim:
            raise ValueError(f"Agent {agent_name} has different state dimension {batch['agent_states'][agent_name].shape[-1]} than other agents {state_dim}")
...
def main(args):
    ...
    infos = get_agent_info(args, env_kwargs_for_eval, agent_obs_rules, make_collate_fn(args, cameras, device))
    agent = MAPPOAgent(**infos, normalize_state=args.normalize_state, use_depth=False)
    ...
    while global_steps < step_infos['total_steps']:
        ...
        agent.train()
        stats = mappo_update_on_policy(args, agent, optimizer, data, collate_fn, accelerator, get_stage(global_steps, step_infos), writer, -1)
