...
class OpenVLA(VLAModelInterface):
    ...
    def policy_class(self):
        return MixedTinyVLAAdapterOpenVLASFTAgent
```

## Step 2: Define the agents and dimensions

Use the agent names and dimensions returned by the selected workload's
`get_agent_info()`.
...
## Step 4: Instantiate OpenVLA

```python
openvla = OpenVLA(
    model_dir="ckpt/vla_adapter_openvla",
    agent_spec=spec,
)
```
