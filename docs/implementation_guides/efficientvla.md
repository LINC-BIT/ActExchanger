...
The third branch of the selected workload policy is EfficientVLA.
...
class EfficientVLA(VLAModelInterface):
    ...
    @property
    def model_name(self):
        return "vla_adapter_smolvla_efficientvla"
    ...
    def policy_class(self):
        return MixedTinyVLAAdapterSmolVLAEfficientVLAAgent
```

## Step 2: Define the agents and dimensions

Keep the workload order `VLA-Adapter`, `SmolVLA`, and `EfficientVLA`.
...
Add these methods to `EfficientVLA`:
...
```python
    ...
    def configure_trainable_modules(self, policy, *, freeze_vla_backbone):
        for name in ("vla_actor", "smolvla_actor", "efficientvla_actor"):
...
## Step 4: Instantiate EfficientVLA

```python
efficientvla = EfficientVLA(
    model_dir="ckpt/vla_adapter_smolvla_efficientvla",
    agent_spec=spec,
)
```
