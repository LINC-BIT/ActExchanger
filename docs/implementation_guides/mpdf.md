...
Bind the low-level agent model to the workload planner. MPDF augments its
deliberation with the meta-actions `persist`, `refine`, and `concede`.
...
class MPDFPlannerModel(PlannerModelInterface):
    planner_name = "mpdf_subtask_planner"
...
class MPDFCommunication(CommunicationInterface):
    communication_name = "mpdf_deliberation_message"
...
Retain each meta-action and its log probability, `group_ids`, deliberation
scores, and the targets produced by `build_softrank_targets` in the rollout.

## 4. Continual Online RL Interface

```python
from api.marl_online_rl_interface import CallbackMARLOnlineRL, run_continual_online_rl
from train.marl.mpdf.base import collect_rollout, mpdf_update_on_policy
...
marl_method = CallbackMARLOnlineRL(
    ...
    update_fn=mpdf_update_on_policy,
)
