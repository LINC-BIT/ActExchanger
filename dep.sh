...
CONTAINER_NAME=${CONTAINER_NAME:-actexchanger-ae}
TYPE=${TYPE:-default}
...
CONTAINER_VENV_DIR=${CONTAINER_VENV_DIR:-/opt/actexchanger-venv}
SMALL_IMAGE_SENTINEL=${SMALL_IMAGE_SENTINEL:-/opt/actexchanger-venv/.actexchanger-ready}
...
create_container() {
    ...
    docker create \
        ...
        -e ACTEXCHANGER_REPO_DIR="$CONTAINER_REPO_DIR" \
        -e ACTEXCHANGER_VENV_DIR="$CONTAINER_VENV_DIR" \
...
exec docker exec -it \
    -e ACTEXCHANGER_IN_CONTAINER=1 \
    -e PS1='(actexchanger-container) \u@\h:\w\$ ' \
    "\$CONTAINER_NAME" \
    bash -lc "printf '%s\n' '==== ActExchanger Docker container ====' 'Container: \$CONTAINER_NAME' 'Workdir: \$CONTAINER_REPO_DIR' 'Type exit to return to the host shell.' '==================================='; if [[ -f "\$CONTAINER_VENV_DIR/bin/activate" ]]; then source "\$CONTAINER_VENV_DIR/bin/activate"; fi; cd "\$CONTAINER_REPO_DIR"; exec bash -i"
START_EOF
